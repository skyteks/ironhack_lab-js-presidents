...
function getNames(presidentsArr) {
    return presidentsArr.map((element) => element.name);
}
...
function countYearsInOffice(presidentsArr) {
    return presidentsArr.reduce((sum, element) =>
        sum += (element.leftOffice === null ? 0 : element.leftOffice - element.tookOffice)
    , 0);
}
...
function sortPresidentsByBirthYear(presidentsArr) {
    return presidentsArr.sort((element1, element2) =>
        element1.birthYear - element2.birthYear
    );
}
...
function getAgeAtInauguration(presidentsArr) {
    return presidentsArr.map(function foobar(element){
        return { ...element, ageAtInauguration: (element.tookOffice - element.birthYear)} 
});
}
...
function countRepublicanPresidents(presidentsArr) {
    return presidentsArr.reduce((sum, element) =>
        sum += element.party == "Republican" ? 1 : 0
    , 0);
}
